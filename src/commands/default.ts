  ...
  .command<{
    ...
    async (argv) => {
      ...
      if (cont && fs.existsSync(convoFp)) {
        conversation += await generateConversation(convoFp);
      } else if (read !== undefined) {
        conversation += await generateConversation(read || append || write);
      }
      ...
      try {
        do {
          ...
          let response = "";
          let prompt = await generateConversation(currentInput);
